...
class PlotSim:
    ...
    def setup(self):
        ...
        self.stem_pcaa_ce = os.path.expanduser('~') + '/FlexCE_v0.0/'
...
#                                'apogee_redclump_v402.pck')
    ...
    def plot_xfe(self,
        ...
        for i, key in enumerate(runs):
            ...
            if pts:
                xs = []
                ys = []
                for tstep in xrange(1, sim['box'].n_steps):
    ...
    def weighted_kde(self, data, weights, xs):
        data_wt = np.array([data[i] for i in xrange(len(data))
                            for w in xrange(weights[i])])
    ...
    def plot_rc_alpha(ax, snr150=False):
        ...
        ax.contour(X, Y, Z, levels=[levels[0], levels[1]], origin='lower',
                   colors='k', zorder=10)
        for i in xrange(nbins_x):
            for j in xrange(nbins_y):
                if Z[j, i] <= sig1 * 1.2:
                    ind_tmp0 = np.where(
